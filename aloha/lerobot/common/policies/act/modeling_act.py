...
class ACT(nn.Module):
    ...
    def forward(self, batch: Dict[str, Tensor]) -> Tuple[Tensor, Union[Tuple[Tensor, Tensor], Tuple[None, None]]]:
        ...
        if self.config.use_vae and "action" in batch: # NOTE:  action shape: [8, 100, 14]
            # Prepare the input to the VAE encoder: [cls, *joint_space_configuration, *action_sequence].
            cls_embed = einops.repeat(
                self.vae_encoder_cls_embed.weight, "1 d -> b 1 d", b=batch_size
            )  # (B, 1, D)
            if self.use_input_state:
                robot_state_embed = self.vae_encoder_robot_state_input_proj(batch["observation.state"]) # [8, 14]
            ...
            latent_sample = mu + log_sigma_x2.div(2).exp() * torch.randn_like(mu) # [8, 32]
        else:
        ...
        actions = self.action_head(decoder_out) # linear, map 512 to 14
