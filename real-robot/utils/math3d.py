...
def quat_to_norm_rpy(quat):
    R = Rotation.from_quat(quat)
    euler = sensitive_gimble_fix(R.as_euler("xyz", degrees=True))
    euler = np.deg2rad(euler)
    rpy_norm = normalize_rad(euler)
    return rpy_norm

def quat_to_degree_rpy(quat):
    R = Rotation.from_quat(quat)
    euler = sensitive_gimble_fix(R.as_euler("xyz", degrees=True)) + 180
    euler = np.deg2rad(euler)
    euler = np.rad2deg(euler).astype(int)
    return euler
